...
def _run_primers_stage(paths: PipelinePaths, args) -> None:
    ...
    if not windows:
        raise PrimerCliError("Primers stage: no candidate windows were generated")
    ...
    if not filtered:
        raise PrimerCliError("Primers stage: no single primers passed basic filters")
    ...
    if not single_cov:
        raise PrimerCliError("Primers stage: no single primers after MSA coverage check")
    ...
    if not pairs:
        raise PrimerCliError("Primers stage: no forward/reverse primer pairs were built")
    ...
    if not pair_cov:
        raise PrimerCliError("Primers stage: no primer pairs passed pair coverage checks")
    ...
    if not scored:
        raise PrimerCliError("Primers stage: no scored primer pairs available")
    ...
    if not final_rows:
        raise PrimerCliError("Primers stage: no final primer rows to export")
